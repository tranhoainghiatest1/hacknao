const { getPool, getStatus, seedDefaultData, getMemoryStore, saveMemoryStore, loadMemoryData } = require('../config/db');

/**
 * Lấy danh sách từ vựng có hỗ trợ Search, Filter, Sắp xếp và Phân trang
 */
exports.getVocabularies = async (req, res) => {
  try {
    ...
    // --- CASE 2: FALLBACK KHI ĐANG CHỜ XAMPP ---
...
exports.createVocabulary = async (req, res) => {
  try {
    ...
    if (!word || !word.trim()) {
      return res.status(400).json({ success: false, message: 'Từ vựng tiếng Anh (word) không được để trống.' });
    }
...
/**
 * Lấy danh sách các Unit và Category phân loại
 */
...
/**
 * Khôi phục / Nạp lại dữ liệu gốc
 */
exports.resetDefaultData = async (req, res) => {
  try {
    const pool = await getPool();
